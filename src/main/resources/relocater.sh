...
if [ ! -z $path ]; then
    ...
    sudo sed '29 c\ \t \t driver = new WebDrivers("Lnx","'$path'").getDriver("CH",8.0,false);' Script.jki
  else
  	sudo cp -R * $path
  ...
  if [ -d /opt/resources/ ]; then
    ...
    sudo sed '29 c\ \t \t driver = new WebDrivers("Lnx","/opt/resources/").getDriver("CH",8.0,false);' Script.jki
  else
